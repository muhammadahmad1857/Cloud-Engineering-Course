...
async function startTimer() {
  const { duration } = await inquirer.prompt([
    {
      ...
      name: "duration",
      message:
        "Enter the duration of the countdown timer (in seconds(only write duration no symbols), minutes(m), hours(h), days(d), months(M), years(y), or as YYYY-MM-DD HH:mm:ss):",
    },
  ]);
  ...
  if (!isNaN(duration)) {
    ...
    countdownDate = new Date(
      now.getFullYear(),
      now.getMonth() + parseInt(duration),
      now.getDate()
    ).getTime();
    ...
    const now = new Date();
    countdownDate = new Date(
      now.getFullYear() + parseInt(duration),
      now.getMonth(),
      now.getDate()
    ).getTime();
  } else {
