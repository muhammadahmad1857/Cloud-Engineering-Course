...
class StudentManagementSystem {
  students: {
    [studentID: string]: {
      name: string;
      courses: { [courseName: string]: boolean }; // Track fee payment status for each course
      balance: number;
    };
  };
  ...
  async addBalance(): Promise<void> {
    ...
    if (this.students[studentID]) {
      ...
      console.log(
        `${this.students[studentID].name} added $${amount} to their balance.`
      );
    } else {
      console.log("Student not found.");
    }
  ...
  async payTuition(): Promise<void> {
    ...
    if (this.students[studentID]) {
      const student = this.students[studentID];
      const courses = Object.keys(student.courses).filter(
        (course) => !student.courses[course]
      );
      if (courses.length === 0) {
      ...
      if (student.balance >= amount) {
        ...
        console.log(
          `Payment of $${amount} received for ${student.name} (${selectedCourse}). Remaining balance: $${student.balance}`
        );
      } else {
        console.log(
          "Insufficient balance to pay tuition fee. First add balance by selecting add balance."
        );
      }
    } else {
      console.log("Student not found.");
