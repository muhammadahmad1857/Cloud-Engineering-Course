...
async function startGame() {
  const startAnimation = chalkAnimation.rainbow(
  ...
  setTimeout(async () => {
    ...
    console.log(
      chalk.blue(
        `I have thought of a number between 1 and ${difficulty.range}. You have ${roundsPlayed} rounds to guess it.`
      )
    );
    askToGuess();
  }, 3000);
...
async function chooseDifficulty(): Promise<Difficulty> {
...
async function askToGuess() {
  if (roundsPlayed > 0) {
    ...
    const answer = await inquirer.prompt([
      {
        ...
        message: chalk.yellow(
          `Guess a number between 1 and ${difficulty.range} in ${roundsPlayed} rounds ->`
        ),
      },
    ]);
    ...
    const endAnimation = chalkAnimation.pulse(
...
function checkGuess(guess: number) {
  if (guess === randomNumber) {
    const congratsAnimation = chalkAnimation.karaoke(
      `Congrats! The secret number is ${guess}. You got it in ${
...
function giveHint(guess: number) {
