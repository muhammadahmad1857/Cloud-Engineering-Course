...
let balance = Math.floor(Math.random() * (20000 - 1000 + 1) + 1000);
let id = 1234;
let pin = 4321;
const transactions = [];
const withdrawMoney = (amount) => {
    if (amount > balance) {
        ...
        balance -= amount;
        console.log(`You withdraw $${amount} and your remaining balance is $${balance}`);
        transactions.push({
            type: "withdrawal",
            amount: amount,
...
const makeTransaction = (amount, receiverId) => {
...
const viewTransaction = () => {
    ...
    transactions.forEach((transaction, index) => {
        console.log(`${index + 1}. ${transaction.type} of $${transaction.amount} on ${transaction.date} and your updated balance is $${transaction.balance}`);
    });
};
...
const userPin = loginQuestions.userPin;
const userId = loginQuestions.userId;
