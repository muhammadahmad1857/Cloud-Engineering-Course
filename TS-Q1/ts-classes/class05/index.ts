...
function calculatePercenage(): void {
  let obtainedMarks: number = 1170;
  let totalMarks: number = 1200;
  console.log(
    `If user obtain ${obtainedMarks} out of ${totalMarks} then it's percentage is ${(
...
function calculatePercentageWithParam(
...
// optional parameter
const userNameWithOptionalParameters = (
  firstName: string,
  lastName?: string
) => {
  if (lastName) {
...
const userNameWithDefaultParameters = (
  firstName: string,
  lastName: string = "Sarwar"
) => {
  return firstName + " " + lastName;
};
