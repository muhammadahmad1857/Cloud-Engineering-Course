...
const checkDaysInMonth = (monthNo: number): void => {
  switch (monthNo) {
    ...
    case 2:
      console.log(
        `The month No. ${monthNo} has 28 days(asssuming non-leap year)`
      );
      break;
    case 4:
    case 6:
...
const calculatePercentage = (marks: number, totalMarks: number): void => {
  let obtainedMarks: number = Math.min(marks, totalMarks);
  let totalMark: number = Math.max(marks, totalMarks);
  let percentage: number = (obtainedMarks / totalMark) * 100;
  console.log(
    `if the user obtain ${obtainedMarks} out of ${totalMark} , The user will get ${percentage.toFixed(
      2
    )} percentage`
  );
};
let obtainedMarks: number = 12000;
let totalMark: number = 11000;
